...
function input(){
	...
	if(isData(n_day) != "ok"){
		...
		if(!obj[0].checked){
			var InGameTime = "IN GAME TIME: " + n_year + "年" + month + "月" + day + "日 " + weekname[(n_day - 1) % 7] + " " + timeH + ":" + timeM;
		}
		else{
		...
		var now = new Date();
		if(!obj[1].checked){
			var RealityTime = "REALITY TIME: " + now.getFullYear() + "年" + (now.getMonth() + 1) + "月" + now.getDate() + "日 " + weekname[now.getDay() - 1] + " " + now.getHours() + ":" + now.getMinutes();
		}
		else{
			var RealityTime = "REALITY TIME: " + now.getFullYear() + "年" + (now.getMonth() + 1) + "月" + now.getDate() + "日 " + weekname[now.getDay() - 1];
		}
